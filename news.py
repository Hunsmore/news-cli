        ...
        if path.exists():
            ...
            if lang == "nhk" or lang == "nhk-more":
                ...
                for tag in tags:
                    ...
                    if (
                        r is not None
                        and r.startswith("/" + sub + "/story")
                        and len(tag.get_text().strip()) > 0
                    ):
                        href = tag.get("href")
                        links.append(href)
                ...
                for tag in tags:
                    ...
                    if (
                        r is not None
                        and (
                            r.startswith("https://www.rtve.es/noticias/")
                            and "more" not in sys.argv
                            or r.startswith("https://www.rtve.es/")
                            and "more" in sys.argv
                        )
                        and r.endswith(".shtml")
                        and len(tag.get_text().strip()) > 0
                    ):
                        href = tag.get("href")
                        links.append(href)
                ...
                if len(lang) <= 4:
                    ...
                    sub = lang[5:]
                res = requests.get(
                    "https://www.france24.com/" + sub + "/", headers=headers
                )
                soup = BeautifulSoup(res.text, "html.parser")
                tags = soup.find_all("a")
                links = []
                for tag in tags:
                    ...
                    if (
                        r is not None
                        and r.startswith("/" + sub + "/")
                        and len(r) > 50
                        and len(tag.get_text()) > 0
                    ):
                        href = tag.get("href")
                        links.append(href)
                        titles.append(tag.get_text().strip())
                ...
                for tag in tags:
                    ...
                    if (
                        r is not None
                        and r.startswith("https://news.detik.com/berita")
                        and "/d-" in r
                        and len(tag.get_text().strip()) > 0
                    ):
                        href = tag.get("href")
    ...
    if cmd == "show":
        ...
        if path.exists():
            ...
            if lang == "nhk" or lang == "nhk-more":
                ...
                summary = (
                    soup.find("p", {"class": "content--summary"}).get_text().strip()
                )
                ...
                paragraphs = soup.find(
                    "div", {"class": "field-type-paragraphs"}
                ).find_all("p")
                ...
                pargs.append(brief.get_text().strip())
                paragraphs = soup.find("div", {"class": "t-content__body"}).find_all(
                    "p"
                )
                for paragraph in paragraphs:
                    text = paragraph.get_text().strip()
                ...
                tit = title.get_text().strip()
                paragraphs = soup.find(
                    "div", {"class": "odd field-item article"}
                ).find_all("p")
                for paragraph in paragraphs:
                    text = paragraph.get_text().strip()
                    if text is not None and text != "":
                ...
                paragraphs = soup.find("div", {"id": "content-body"}).find_all(
                    re.compile("^((p)|(blockquote)|(h2)|(li))$")
                )
                for paragraph in paragraphs:
                    text = paragraph.get_text().strip()
